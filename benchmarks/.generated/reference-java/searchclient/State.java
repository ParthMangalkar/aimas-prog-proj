    ...
    */
    ...
    private State(State parent, Action[] jointAction)
    {
        ...
        for (int agent = 0; agent < numAgents; ++agent)
        {
            ...
            switch (action.type)
            {
                ...
                case Push:
                    // Agent moves into box's current cell; box moves one cell further.
                    int pushBoxRow = this.agentRows[agent] + action.agentRowDelta;
                    int pushBoxCol = this.agentCols[agent] + action.agentColDelta;
                    int pushBoxNewRow = pushBoxRow + action.boxRowDelta;
                    int pushBoxNewCol = pushBoxCol + action.boxColDelta;
                    // Move box
                    this.boxes[pushBoxNewRow][pushBoxNewCol] = this.boxes[pushBoxRow][pushBoxCol];
                    this.boxes[pushBoxRow][pushBoxCol] = 0;
                    // Move agent into box's old cell
                    this.agentRows[agent] += action.agentRowDelta;
                    this.agentCols[agent] += action.agentColDelta;
                    break;

                case Pull:
                    // Agent moves away; box follows into agent's old cell.
                    // boxRowDelta/boxColDelta = direction the box moves (toward agent's old cell).
                    // Box's current position = agent_pos - boxDelta.
                    int pullBoxRow = this.agentRows[agent] - action.boxRowDelta;
                    int pullBoxCol = this.agentCols[agent] - action.boxColDelta;
                    // Move box into agent's old position
                    this.boxes[this.agentRows[agent]][this.agentCols[agent]] = this.boxes[pullBoxRow][pullBoxCol];
                    this.boxes[pullBoxRow][pullBoxCol] = 0;
                    // Move agent
                    this.agentRows[agent] += action.agentRowDelta;
                    this.agentCols[agent] += action.agentColDelta;
                    break;
    ...
    private boolean isApplicable(int agent, Action action)
    {
        ...
        switch (action.type)
        {
            ...
            case Push:
                // The box is adjacent to the agent in the agent's move direction.
                boxRow = agentRow + action.agentRowDelta;
                boxCol = agentCol + action.agentColDelta;
                box = this.boxes[boxRow][boxCol];
                if (box == 0)
                {
                    return false; // No box there
                }
                // Agent and box must be same color
                if (agentColor != boxColors[box - 'A'])
                {
                    return false;
                }
            ...
            case Pull:
                ...
                // The box is at agent - boxDelta (opposite of box movement direction)
                ...
                // Agent and box must be same color
                return agentColor == boxColors[box - 'A'];
        }
    ...
    private boolean isConflicting(Action[] jointAction)
    {
        ...
        for (int agent = 0; agent < numAgents; ++agent)
        {
            ...
            switch (action.type)
            {
                ...
                case Push:
                    // Agent moves to box's cell; box moves further
                    destinationRows[agent] = agentRow + action.agentRowDelta;
                    destinationCols[agent] = agentCol + action.agentColDelta;
                    boxRow = destinationRows[agent] + action.boxRowDelta;
                    boxCol = destinationCols[agent] + action.boxColDelta;
                    boxRows[agent] = boxRow; // Box destination
                    boxCols[agent] = boxCol;
                    break;

                case Pull:
                    // Agent moves away; box follows to agent's old cell
                    destinationRows[agent] = agentRow + action.agentRowDelta;
                    destinationCols[agent] = agentCol + action.agentColDelta;
                    boxRows[agent] = agentRow; // Box destination = agent's old cell
        ...
        for (int a1 = 0; a1 < numAgents; ++a1)
        {
            ...
            for (int a2 = a1 + 1; a2 < numAgents; ++a2)
            {
                ...
                // Box destinations collide?
                if (boxRows[a1] == boxRows[a2] && boxCols[a1] == boxCols[a2])
                {
                    return true;
                }
                // Agent destination == other's box destination?
